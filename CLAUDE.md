...
The five deployed pieces: **`vocabulaire_hebreu.html`** (the notebook — generated by `node build.js` from `data/*.json` + `src/carnet/`; never edit it directly), **`index.html`** (the portal / root page), **`app.html`** (the online flashcards, fetches `cards.json` at load), **`flashcards_hebreu.html`** (the standalone offline build, cards inlined by the same build), and the PWA layer (`manifest.webmanifest`, `sw.js`, `icons/`). `data/*.json` is the single source of truth for all content. Ask the graph for what any of them contains; ARCHITECTURE.md holds the prose version.
...
1. **Never edit `flashcards_hebreu.html`, `vocabulaire_hebreu.html` or `cards.json` by hand.** All three are 100% generated by `node build.js` from `data/*.json` + `src/carnet/` (the standalone also folds in `app.html`). Hand edits are erased on the next build.
...
1. `node build.js` — reads `data/*.json`, regenerates the notebook, `cards.json` and the standalone; fails if a section or an `EXPECTED_LEVELS` level drops to 0, if any entry lacks a valid `niveau`, or if a table entry lacks `theme` / carries a slug outside `EXPECTED_THEMES`. Check the printed counts.
