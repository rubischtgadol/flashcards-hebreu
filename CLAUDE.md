...
1. **Never edit `flashcards_hebreu.html`, `vocabulaire_hebreu.html`, `cards.json` or `app.html` by hand.** All four are 100% generated by `node build.js` from `data/*.json` + `src/carnet/` + `src/app/` (the standalone is derived from the app assembled in memory, never from the `app.html` on disk). Hand edits are erased on the next build; `node build.js --check` compares all four.
...
- The card schema is unchanged in shape (`{cat, he, tr, fr, note?, niveau?, theme?, exemples?:[{he, tr, fr, he_plain}], he_plain, forms?:[{he, tr, label, he_plain}], genre?}`) — its single source is now `deriveCartes(donnees)`. Table-derived cards have `tr:''`; the UI falls back to `he2tr(card.he)`. Full detail in ARCHITECTURE.md §3.
...
1. `node build.js` — reads `data/*.json` + `src/`, regenerates the **four** artefacts (notebook, `cards.json`, `app.html`, standalone); fails if a section or an `EXPECTED_LEVELS` level drops to 0, if any entry lacks a valid `niveau`, or if a table entry lacks `theme` / carries a slug outside `EXPECTED_THEMES`. Check the printed counts.
...
- **`ARCHITECTURE.md`** — data flow, the gabarits/data contract, card schema, CEFR levels, the validator's lexicon and its two guards, `app.html`'s anatomy, the silent-breakage guards. **§ « Le graphe de connaissance du dépôt » documents the graph itself.**
