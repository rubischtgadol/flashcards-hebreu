...
**Aucune dépendance, aucun test, aucun gestionnaire de paquets.** Chaque fichier déployé est un document HTML autonome (CSS et JS inline, vanilla). Le seul outillage est `tools/build.js`, `tools/verifie_exemples.js`, `tools/ajoute_mots.js` et `tools/cherche_mots.js`, quatre scripts Node zéro-dépendance, utilisés uniquement en développement et jamais déployés. Ils vivent dans `tools/` depuis le chantier 4 et **se lancent depuis la racine du dépôt** : chacun vise `ROOT = path.join(__dirname, '..')`, exporté par `build.js` pour que les trois autres ne le recalculent jamais.
...
Il n'y a donc **qu'une seule app** (les sources de `src/app/`, la racine étant un portail léger) et **qu'une seule source de contenu** (`data/*.json`). Le carnet, `cards.json`, `app.html` et le fichier autonome sont quatre projections mécaniques de ces sources, produites par `node tools/build.js`.
...
ce qui est **servi** (les quatre artefacts, `index.html`, la couche PWA) plus `README.md`
et `CLAUDE.md`. Les sources sont dans `data/` (contenu) et `src/` (gabarits et code de
l'app) ; l'outillage dans `tools/` ; la prose dans `docs/`. **Les outils se lancent
...
## Anatomie de l'app (sources `src/app/`, artefact `app.html` ~2410 lignes)
...
Les quatre premiers filets détectent les cartes perdues ; les quatre suivants (lot tripwires du 2026-07-25 et Task 17, chaque garde éprouvée par un cas de casse fabriqué avant d'être crue) attrapent la casse de charte, la preuve muette et le commit incomplet :
...
5. **`verifieCharte()` dans `build.js`** (25/07) : mécanise les pièges n°2, 3 et 5 de CLAUDE.md, fatale en mode normal comme en `--check` — `transition:…all` interdit dans l'app assemblée (WebKit fige les longhands `outline-*`, l'anneau de focus disparaît sans symptôme), aucun `font-size` sur le sélecteur `html` (app et carnet), le bloc `:root` d'`index.html` doit contenir `src/tokens.css` verbatim (copie manuelle jusqu'au Task 18), et `--bg` doit se retrouver dans `manifest.webmanifest` (`theme_color`, `background_color`) et dans chaque `<meta name="theme-color">` — le message rappelle que les icônes, elles, ne sont vérifiables par aucune commande. ⚠️ Le scan CSS ne parcourt que les contenus `<style>` : appliqué au document entier, le regex de blocs était quadratique sur les longs runs HTML sans accolade (build gelé, payé à l'installation du lot).
...
8. **Hook `pre-commit` versionné** ([.githooks/pre-commit](../.githooks/pre-commit), 25/07 ; installation par machine : `git config core.hooksPath .githooks`) : exécute `node tools/build.js --check` et `node tools/verifie_exemples.js` avant chaque commit, et refuse un commit qui change un fichier servi (`app.html`, les artefacts, `index.html`, `manifest.webmanifest`, `sw.js`, `icons/`) sans bump de `VERSION` dans `sw.js` (piège n°10 ; bypass assumé : `--no-verify`). Tripwire de transition — le Task 19 fera estampiller `VERSION` par le build.
