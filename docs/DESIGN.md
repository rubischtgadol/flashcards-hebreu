...
**La rampe du carnet, et le socle qu'elle corrige.** `vocabulaire_hebreu.html` déclarait **24 tailles distinctes pour 52 déclarations** (`1.12rem`, `0.92rem`, `0.82rem`, `1.35rem`…). Ce n'était pas de la négligence : c'était le symptôme d'un socle mal compris, et il faut connaître la cause avant de toucher aux valeurs.
...
⚠️ **`--colonne-large` est un plancher mesuré, pas un confort.** Les tables se posent à **894px** et le resserrer à 44rem « puisque les tables portent `min-width:640px` » en met plusieurs en défilement horizontal : ce `min-width` est lui aussi un plancher, qui ne joue que sur mobile. Essayé, mesuré, annulé — deux fois, aux lots suivants. ⚠️ Et la leçon que ces recalages ont coûtée : la phrase « aucune table ne dépasse 894px » est restée écrite ici alors qu'un lot venait d'en produire une à 909,94px (« Vie quotidienne », Verbes, corrigée depuis). **Elle décrivait une mesure, pas une garde** — aucune commande ne la vérifiait, donc rien n'a signalé qu'elle était devenue fausse. Un chiffre consigné dans la prose se périme en silence : à remesurer (suite de mise en page, RITUEL.md § Outillage) après tout lot de vocabulaire.

⚠️ **La largeur d'une table est la somme des min-contents de ses colonnes, jamais la largeur d'un contenu.** Les tables se posent à 894 parce que `table{width:100%}` les remplit ; une seule devient *min-content-bound*, et ce jour-là sa largeur cesse d'obéir au cadre. Trois pistes ont été essayées et mesurées avant la bonne, chacune fausse pour la même raison — elles traitaient 909,94 comme une propriété de la table :

- **relever `--colonne-large`** : déplacerait sommaire, titres à filet et toutes les tables pour arranger un cas (piège n°4) ;
- **laisser la translittération revenir à la ligne** (`white-space:normal` sur `.tr` en cellule) : **inopérant**, `mitmake'ach` est un mot sans espace, donc son min-content reste le mot entier ; il aurait fallu `word-break:break-all`, c'est-à-dire couper un guide de prononciation en pleine syllabe ;
- **rogner les phrases d'exemple** de la colonne Infinitif (390,63px, le plus gros poste) : la colonne est bornée par une **pile, pas par un pic** — la plus large ne rend que 7,72px puis la suivante reprend la main (paliers mesurés : 902,22 / 899,61 / 896,02 / 894,00). Atteindre le seuil demandait de réécrire **3 exemples entiers sur 3 lignes différentes** pour sauver 1 mot.

Le vrai coupable était une seule cellule : la translittération `mitmake'ach` (105,61px) dans une colonne de conjugaison dont les pairs plafonnent à 86,41. **Un déséquilibre local, pas une largeur absolue** — le carnet porte ailleurs des contenus bien plus larges (326,41px pour une translittération d'exemple dans `שֶׁ־` subordination, 280,58px pour un `.fr` d'Adjectifs) qui ne débordent pas, parce que leurs colonnes les absorbent. D'où la garde : **une forme conjuguée dont la translittération dépasse ~90px ne tient pas dans une table à 5 colonnes**, et le standard interdit de la raccourcir. `לְהִתְמַקֵּחַ` (marchander) a donc été remplacé par `לְהַחְלִיף` (échanger, `machlif` à 67,20px) — un mot sorti du carnet parce qu'aucune retouche ne pouvait le faire entrer.
...
Sous 640 px, les **tables de vocabulaire** — reconnues à leurs exemples embarqués (`.table-wrap:has(ul.exemples)`) — quittent la grille défilante : chaque rang devient une carte pleine largeur (rayon 11px « rang », filet) disposée en **flex-colonne réordonnable** — le mot-vedette d'abord, puis les formes courtes **sur une ligne**, chacune sous son étiquette mono (voix Label : GENRE/PLURIEL, MS/FS/MP/FP — les thead masqués), et **l'exemple EN DESSOUS** : l'ordre de lecture que Ruben attend, pas celui du DOM. Le geste qui le permet : la première cellule se **dissout** (`display:contents`) pour que ses enfants (`he`/`cursive`/`fr`/`ul.exemples`) deviennent les items flex du rang et se laissent replacer par `order` (vedette 1-3, formes 4, exemple 5), **sans toucher au markup dont dépend l'extraction**. Le nombre de colonnes signe toujours la catégorie (3 = Noms, 4 = Adjectifs, 5 = Verbes), donc les étiquettes tiennent en CSS pur. ⚠️ Nuance assumée : une ligne de formes qui déborde la carte **passe à la ligne** plutôt que de défiler — un vrai défilement de sous-ligne exigerait un conteneur d'enrobage que le couplage d'extraction interdit, et un défilement horizontal enfermé dans le vertical est le pire geste tactile. Les **tables de grammaire** (celles sans exemples) gardent leur grille : leur structure *est* la leçon. Motif : sur iPhone, les tables s'ouvraient sur leur **fin** (`table{direction:rtl}` dans un wrap LTR — le scroll initial montrait le pluriel d'un mot hors-champ) et imposaient le pire geste du tactile ; le wrap est passé `direction:rtl` (l'origine de défilement suit la table), ce qui règle aussi les grilles conservées.

⚠️ **Le piège de `display:contents` : il supprime la boîte, pas l'héritage.** La cellule dissoute laisse passer le `white-space:nowrap` de la règle générique `th,td` vers `.he`, `.cursive`, `.fr` et tout le contenu de `ul.exemples` — du texte qui doit précisément pouvoir revenir à la ligne. Tant que le rang restait dans la grille défilante, c'était sans conséquence : le wrap coupait. Mais la carte mobile passe en `overflow-x:visible`, donc plus rien ne coupe et **le texte insécable peint hors de l'écran**. Une cause, deux symptômes sans ressemblance apparente : la glose française d'une entrée sortait par la **gauche** de sa carte (`left −31px`, texte coupé), et les `.fr`/`.tr` des phrases d'exemple — eux en `direction:ltr` dans un rang RTL — sortaient par la **droite** et poussaient le document à 444 px pour 402 de viewport. La cellule voisine (`td:not(:first-child)`) posait déjà `white-space:normal` de son côté ; le correctif est la même remise à zéro, appliquée à la cellule que `display:contents` avait rendue invisible à la règle.

⚠️ **Et un piège de mesure, qui a fait conclure deux fois de travers avant d'être vu.** Ce débordement **ne se voit pas** sur `getBoundingClientRect()` des éléments : sous flex, les boîtes se rétrécissent normalement (`flex-shrink:1`, `min-width:0`) et seul le **texte** dépasse, sans agrandir le rect de son élément. Une passe qui parcourt `document.querySelectorAll('*')` trouve donc **zéro débordant à droite** pendant que `scrollWidth` vaut 444. Il faut mesurer au **`Range` sur les nœuds texte** : 6 débordements, le pire à `right = 444,1` — qui colle au `scrollWidth` au dixième de pixel. Corollaire : un filtre « élément non contenu par un ancêtre scrollable » désigne un coupable plausible et faux (ici la glose, qui débordait à gauche et ne pouvait donc rien ajouter à `scrollWidth`).
...
**Un pixel suffisait, et notre banc ne le voyait pas.** La hauteur au contenu a bien réglé le débordement — au pixel près, littéralement : la carte tombe *exactement* sur la hauteur de son contenu, sans coussin. Or `clientHeight` et `scrollHeight` n'arrondissent pas la même fraction du même côté selon le moteur. WebKit rend 152 / 152 ; Blink rendait 152 / 153. Ce pixel-là est invisible sous WebKit, dont les barres de défilement sont en surimpression, et défigurant sous Windows où elles sont classiques : elles apparaissent dès le **premier** pixel de dépassement, avec leurs flèches, et volent 15 px de largeur au contenu. D'où `#face-content{overflow:visible}` au-delà de 900 px — là où la carte prend la hauteur de son contenu, il n'y a rien à faire défiler, donc rien à équiper d'une barre. Le téléphone garde `overflow-y:auto` : son plafond `32vh` rogne pour de vrai.

**Le palier avait un angle mort, et il était plus large qu'une respiration : trois règles écrites, aucune appliquée.** `body.input-mode #face-content .forms` — marge, gouttière et taille des inflexions — vivait dans le `@media (min-width:900px)` de `30-cartes.css` et **n'a jamais rien fait**. Sa règle de base est dans `40-reponses.css`, concaténé *après* : à sélecteur égal c'est le dernier qui gagne, et **une media query n'ajoute pas de spécificité**. Le palier ordi rendait donc les inflexions à 1,7rem et leur marge à 12 px — les valeurs pensées pour la carte comprimée du téléphone, soit douze pixels entre un mot de 76,8 px et ses formes. L'override est reposé dans le fichier où vit sa base : marge **24 px** (la valeur que `.forms` porte déjà ici, pas un nombre neuf) et inflexions à **2,2rem**, ce que la règle de la vedette réclamait depuis le début. Et `.hint` gagne 12 px de marge : la carte étant pile à la hauteur du contenu, `align-content:center` n'a aucun mou à répartir et « traduis en français » se collait sous les étiquettes.

⚠️ **La leçon de cascade vaut pour les six fragments CSS de l'app.** Ils sont concaténés dans l'ordre de `src/app/ordre.json` ; un palier écrit dans un fragment ne peut pas surcharger une règle de même sélecteur posée dans un fragment **suivant**, quelle que soit sa media query. Un override appartient au fichier de sa base — ce qui rejoint « un fichier ne fait qu'une chose ». Rien ne le mécanise aujourd'hui : le bloc mort a survécu à un chantier entier sans qu'aucune garde ne bronche.

**Deux colonnes : la carte à gauche, ce qu'on en fait à droite.** Le palier avait rendu la largeur à la carte, mais l'écran d'étude restait une pile — carte, puis champ, puis correction, puis « Suivant ». Mesuré à 1536×730 : le bouton « Suivant » tombait **42 px sous la fenêtre** l'exemple replié, **150 px** déplié, **222 px** sur un verbe à quatre formes ; en mode Cartes, « Je savais » dépassait de 19 px. Et le coupable n'était pas la carte (264 px) mais le bloc de correction (**375 px**) : l'empiler sous un objet qui prend déjà la moitié de la hauteur utile ne pouvait pas tenir, quelle que soit la largeur disponible. Le vide latéral et le débordement vertical étaient — une seconde fois, et pour une raison différente — **le même défaut**.

Au-delà de 900 px, `.study.active` passe donc en grille : `.progress-wrap` enjambe les deux colonnes, `.stage` occupe la première, un conteneur `.answer-col` la seconde. **420 px à droite**, parce que c'est la translittération d'exemple qui dicte cette largeur et que 420 px couvre 96,8 % du corpus — au-delà le texte se replie, il n'est jamais tronqué. Pistes en `minmax(0, …fr)` et non en largeurs fixes : à 900 px il n'y a pas 1094 px à distribuer, et les deux colonnes doivent se resserrer ensemble (506/332) plutôt que déborder. Déficit vertical après : **zéro sur 39 des 42 combinaisons mesurées** ; les trois restantes cumulent une fenêtre de 700 px de haut ou moins, le contenu le plus dense et l'exemple déplié. Mesuré sous WebKit **et confirmé sur Chrome/Windows** — la confirmation compte : le piège n°17 rappelle que le banc ne voit pas ce que ce moteur-là rend.
...
- **Do** écrire les piles de polices **en entier**, jamais `'Frank Ruhl Libre',serif` : les polices Google sont la seule dépendance réseau, donc hors-ligne c'est le fallback qui rend le nikoud — et le `serif` générique le rend nettement moins bien que David Libre. Les **quatre piles normatives**, telles qu'elles s'écrivent dans le code :
  ...
  Corollaire direct du principe « ça marche dans l'avion ». Le carnet et `app.html` ont eu leurs déclarations tronquées (9 mono, 2 Assistant) complétées. **Les trois fichiers écrivent désormais les quatre piles en entier** — un `font-family` qui n'a pas au moins deux replis est un défaut.
