...
⚠️ **Trois pièges relevés au Task 16 (25/07), après l'écriture de ce plan — les lire AVANT le `git mv`** :

1. **`build.js` n'est pas le seul à prendre `__dirname` pour la racine.** `verifie_exemples.js:57` (`chargeDonnees(__dirname)`), `cherche_mots.js:152` et `:155`, et `ajoute_mots.js:55` (`const ROOT = __dirname`) font pareil. Les quatre sont à recaler, pas seulement `build.js` — sinon ils liront `data/` dans `tools/`, c'est-à-dire nulle part.
2. ⚠️ **Le bac à sable d'`ajoute_mots.js` (`sandboxValidation`, ~l. 311-330) casse en silence.** Il copie `build.js` et `verifie_exemples.js` **à plat** dans un `mkdtemp`, à côté de `src/` et des données candidates, puis les lance avec `cwd: dir` — tout repose sur `__dirname === dir`. Avec `ROOT = path.join(__dirname, '..')`, la racine du bac à sable devient `os.tmpdir()` : la validation ne porte plus sur le candidat. **Le bac à sable doit reproduire la disposition `tools/`** (copier les scripts dans `dir/tools/`), et le dry-run d'`ajoute_mots.js` doit être re-prouvé sur un mot fabriqué — un bac à sable qui valide le mauvais arbre passe au vert, comme un témoin muet.
3. **`.githooks/pre-commit` invoque `node build.js --check` et `node verifie_exemples.js`** depuis la racine (créé au lot tripwires du 25/07, donc après ce plan). Le `grep` du Step 2 ci-dessous ne le voit pas : il est borné à `README.md CLAUDE.md docs/ --include=*.md`. **Élargir la recherche aux non-`.md`** : `.githooks/`, `.gitignore`, et tout `require`/`spawn` dans les scripts eux-mêmes.

- [x] **Step 1 :** `git mv`, ajuster `ROOT`, puis `node tools/build.js && node tools/build.js --check && node tools/verifie_exemples.js && node tools/cherche_mots.js --stats` → tout vert. **Fait le 25/07.** `ROOT` est **exporté par `build.js`** et consommé par les trois autres, plutôt que recalculé trois fois. Les quatre artefacts se sont régénérés à l'identique — c'est la preuve que `ROOT` vise juste.
- [x] **Step 2 :** ⚠️ **le `grep` du plan était insuffisant**, comme le piège 3 l'annonçait — et pas seulement sur les non-`.md`. Ce qu'il ratait et qui a dû être corrigé en plus : les **chaînes d'usage et les messages d'erreur dans les quatre scripts eux-mêmes** (19 occurrences de `node build.js`…), dont trois **sortent dans l'en-tête « FICHIER GÉNÉRÉ » des artefacts** (donc rebuild + bump `sw.js` → **v34**) ; l'**allowlist** `.claude/settings.local.json` ; et les liens markdown à fragment (`](src/app/coquille.html#L42)`), invisibles au motif sans `#`. Sweep exhaustif délégué à un sous-agent Sonnet (4 critères chiffrés) : 30 défauts nommés, tous corrigés ; archives (`TODO_ARCHIVE.md`, `docs/superpowers/**`) laissées figées, à dessein.
- [x] **Step 3 :** commit. **Deux corrections hors plan, dues au lot tripwires du 25/07 :** (a) le bac à sable d'`ajoute_mots.js` était **déjà cassé avant le déménagement** (`verifieCharte()` lit `index.html`, jamais copié → ENOENT) — d'où `FICHIERS_RACINE_BAC_A_SABLE` ; (b) ce bac à sable était un **témoin muet** (compte de cartes calculé en process, pas relu de la sandbox) — d'où `assertBacASableCoherent()`. Les deux gardes éprouvées par casse fabriquée. ⚠️ Le piège 2 annonçait une casse silencieuse de la copie à plat : **faux**, elle échoue bruyamment (`Cannot find module '../src/carnet/gabarits.js'`).
...
- [x] **Step 1 :** fait le 25/07. Gate tenue : `index.html` régénéré **byte-identique** à l'actuel, seul l'en-tête « FICHIER GÉNÉRÉ » diffère (`diff` d'une ligne). Le carnet, l'app et le standalone se sont régénérés **inchangés** au même build — c'est la preuve que la factorisation de la lecture des jetons (`lisTokens()`, une lecture pour trois consommateurs) est neutre. `--check` couvre 5 artefacts (4 + `index.html`). Piège 5 clos par construction.
- [x] **Step 2 :** commit. **Trois élargissements hors plan, tous éprouvés par casse fabriquée (9 cas, exit 1 + message nommé) :** (a) `verifieCharte()` reçoit désormais le **portail assemblé** et non plus `index.html` relu du disque, et les contrôles « transition:all » et « font-size sur html » lui sont étendus (il a son propre anneau de focus or et anime ses portes) ; (b) le contrôle « les jetons sont-ils là ? » devient « les jetons sont-ils là **et seuls** ? » — le **nombre de blocs `:root` attendu par page** (carnet 3, app 1, portail 1), parce que l'injection garantit la présence, jamais l'unicité : un second `:root` en dur gagnerait par cascade ; (c) les **trois substitutions de `genereCarnet()`** (`@TOKENS`, `@CSS:carnet`, `@JS:carnet`) passent enfin par `mustReplace` — elles étaient les seules non gardées, un marqueur retiré de `tete.html` amputait le carnet en silence. Effet de bord recalé : `index.html` sort de `FICHIERS_RACINE_BAC_A_SABLE` (`ajoute_mots.js`) puisque plus aucune garde ne le lit du disque — dry-run re-prouvé sur un mot fabriqué (1221 cartes, `data/` intact).
...
- [x] **Step 1 :** `rm -r outils_migration && node tools/build.js --check` → vert. ⚠️ **Le plan se trompait sur les helpers** : le contrôle nom par nom a montré que `parseSections`, `exemplesOf`, `attrOf`, `tdsOf` n'avaient **aucun** appelant interne, et que `closeOf`/`firstSpanText`/`decodeEntities` n'étaient appelés que par ceux-là (plus `textContent`, `blocksOf`, `NAMED_ENTITIES`). Tout le sous-graphe ne tenait que par les exports : supprimé en entier, pas seulement les exports. `--check` vert, artefacts **inchangés** (donc `sw.js` non réestampillé — le mini-parseur ne participait à aucune sortie). Les quatre familles de références élargies (chaînes dans les scripts, allowlist, liens à fragment, bac à sable) : toutes vierges d'`outils_migration`, vérifié.
- [x] **Step 2 :** passe de documentation, par sections (grep du titre puis édition ± 30 lignes, jamais de relecture intégrale) :
...
- [x] **Step 3 :** commit : « Chantier 4 (Task 20) : outils_migration/ supprimé, dernier lecteur de HTML avec ». Le flag graphe posé dans TODO.md dit explicitement qu'**il n'y a rien à retirer côté graphe** (le dossier a été créé après le dernier recalage, le graphe ne l'a jamais connu).
