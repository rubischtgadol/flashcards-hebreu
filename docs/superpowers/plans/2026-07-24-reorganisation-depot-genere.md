...
- [ ] **Step 4 :** supprimer d'`outils_migration/` les fichiers absorbés (`valide_donnees.js`, `genere_carnet.js`) **dans le même commit** — aucune copie de logique ne reste vivante en double (principe 2) ; `extrait_donnees.js`, `decoupe_carnet.js` et `compare_carnets.js` restent jusqu'au Task 20.
...
**Files:**
- Delete: `outils_migration/` — ce qu'il en reste après le Task 7 Step 4 : `extrait_donnees.js`, `decoupe_carnet.js`, `compare_carnets.js` (les contrôles vivent désormais dans `--check` et `--verrou` de `build.js` ; l'historique git garde les scripts jetables)
