// Expose : nivOfCard, nivOk, themeOk, indexRecherche, runSearch, buildChips, buildNivChips, buildThemeChips, toggle, segPick, refreshAdvSub, foldSub, refreshFoldSubs, applyFoldState, refreshSelAll, updateStart, limitPool, THEMES, SPK_SVG, catCounts, nivCounts, themeCounts, catsEl, nivEl, themeEl, catOrder — Utilise : state (99), CARDS (05), escapeHtml (01), he2tr/cleRecherche (02), speak/bindTap (06/11), savePrefs/sessRestore/srsMigrateIds (04), applyPrefs/perfReport (13), refreshSrsUi (08)
...
// L'index de recherche, bâti UNE fois et gardé. Deux raisons de ne pas le
// refaire à chaque frappe : he2tr est un transducteur, et il doit tourner sur
// les cartes issues des tables — qui portent `tr:''` (l'immense majorité). Ce
// manque était le défaut signalé le 2026-07-29 : la botte de foin fouillait
// `c.tr` brut, si bien que la translittération AFFICHÉE juste à côté dans le
// résultat (`c.tr||he2tr(c.he)`, plus bas) était elle-même introuvable.
// ⚠️ L'index est apparié à CARDS par position : on garde la référence du
// tableau et l'on rebâtit s'il a changé, sinon un rechargement des cartes
// laisserait un index périmé qui désignerait les mauvais mots — en silence.
let SEARCH_INDEX = null, SEARCH_INDEX_SRC = null;
function indexRecherche(){
  if(SEARCH_INDEX && SEARCH_INDEX_SRC === CARDS) return SEARCH_INDEX;
  SEARCH_INDEX_SRC = CARDS;
  SEARCH_INDEX = CARDS.map(c=>{
    ...
    // `cle` sert à TROUVER (tout, formes comprises), les trois autres à
    // CLASSER : chercher שלום et lire לְשַׁלֵּם en tête est presque aussi
    // décourageant que ne rien trouver. Le squelette consonantique fait se
    // rencontrer des mots d'une même racine — c'est voulu, mais le mot
    // exactement demandé passe devant.
    return {
      cle: cleRecherche(hay.filter(Boolean).join(' ')),
      he:  cleRecherche(c.he),
      tr:  cleRecherche(c.tr || he2tr(c.he)),
      fr:  cleRecherche(c.fr || '')
    };
  });
  return SEARCH_INDEX;
}
// 2 : le mot demandé, exactement. 1 : le mot commence par ce qu'on a tapé —
// c'est la frappe en cours. 0 : la correspondance est ailleurs (une forme
// fléchie, un exemple, une racine voisine).
function rangResultat(e, q){
  if(e.he === q || e.tr === q || e.fr === q) return 2;
  if(e.he.indexOf(q) === 0 || e.tr.indexOf(q) === 0 || e.fr.indexOf(q) === 0) return 1;
  return 0;
}
function runSearch(){
  ...
  const q = cleRecherche(raw);
  if(!q){ box.innerHTML=''; box.classList.add('hide'); return; }
  box.classList.remove('hide');
  const idx = indexRecherche();
  // On balaie TOUT avant de couper à 80 : s'arrêter au 80e trouvé rendrait le
  // classement inopérant, puisque le mot exactement demandé peut être le 200e
  // rencontré dans l'ordre du corpus.
  const trouves=[];
  for(let i=0;i<CARDS.length;i++){
    if(idx[i].cle.includes(q)) trouves.push({ c: CARDS[i], r: rangResultat(idx[i], q) });
  }
  ...
  let html = '<div class="sr-count">'+total+' résultat'+(total>1?'s':'')
    + (total>hits.length ? ' · les '+hits.length+' plus proches' : '')+'</div>';
  html += hits.map((c,i)=>{
...
const catOrder = ['Pronoms personnels','Démonstratifs','Verbes','Verbes modaux','Impératif','Tournures impersonnelles','Adjectifs','Comparatif et superlatif','Noms','Abréviations et sigles',
  'Prépositions','Prépositions fléchies','Conjonctions','Connecteurs du discours','Mots interrogatifs','Existence','Nombres',
  'Jours de la semaine','Heure et date','Saisons & mois','Adverbes','Mots de quantité','Expressions','Phrases','Hébreu parlé','Hébreu biblique'];
function buildChips(){
  catCounts = {};
