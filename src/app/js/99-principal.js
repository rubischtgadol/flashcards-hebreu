...
async function init(){
  ...
  try{
    ...
    // ⚠️ tB1→tB2 s'appelait « extraction » du temps où l'app lisait le HTML du
    // carnet ; elle ne mesure plus qu'un JSON.parse depuis la bascule sur
    // cards.json. L'étiquette dit désormais ce qu'elle chronomètre.
    const pb=document.getElementById('perf-boot');
    if(pb) pb.textContent='Chargement : cartes (réseau) '+fmtMs(tB1-tB0)
      +' · lecture JSON '+fmtMs(tB2-tB1)+' · construction '+fmtMs(tB3-tB2)
      +' · total '+fmtMs(tB3-tB0);
