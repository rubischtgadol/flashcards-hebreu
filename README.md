...
- **Partie 1 — Lire l'hébreu, puis la grammaire** : elle s'ouvre sur **le nikoud** — les treize signes un par un, avec leur nom, leur prononciation et un exemple tiré du carnet, puis le shva, les chatafs des gutturales, le qamats katan, le dagesh et ses trois rôles, le point du shin, le patach furtif, et pourquoi les Israéliens écrivent sans nikoud au quotidien. Viennent ensuite pronoms, démonstratifs, la racine et **les 7 binyanim** (le sens de chacun, ses signes de reconnaissance, comment s'en servir), le passé, le futur, patrons de conjugaison, **les verbes réfléchis** (hitpa'el et sa métathèse), l'article défini, l'état construit, prépositions fléchies, le hé directionnel, existence & possession, la négation, le « que » et **le « quand » de subordination** (כְּשֶׁ־ face à מָתַי).
- **Partie 2 — Vocabulaire, le dictionnaire** : verbes, noms et adjectifs, regroupés par thèmes.
- **Partie 3 — Mots-outils & expressions** : prépositions, conjonctions, mots interrogatifs, nombres, jours de la semaine, mots de quantité, adverbes (temps, lieu, degré, manière), pronoms réfléchis, expressions courantes, **phrases utiles du quotidien** (« Combien ça coûte ? », « L'addition, s'il te plaît »…) **les abréviations et sigles** — les רָאשֵׁי תֵּבוֹת qu'on croise sur les panneaux et les formulaires (צה"ל, עו"ד, וכו'), avec les trois façons de les lire à voix haute, **l'hébreu parlé** — le registre familier qu'on entend sans le trouver dans les manuels (תַּכְלֶס, כְּאִילוּ, סַבָּבָּה), chaque entrée avec sa phrase d'usage — et **l'hébreu biblique** : le fonds du texte biblique en section dédiée (formes archaïques avec leur équivalent moderne, culte et liturgie, récit et image), jamais mêlé aux tables modernes.
...
- **Diagnostic de latence** : les « Réglages avancés » affichent les millisecondes du chargement et du dernier geste (attente · travail · affichage) — la mesure se lit sur l'appareil même, sans inspecteur.
...
- `tools/` — les sept outils de développement (non déployés, zéro dépendance ; se lancent **depuis la racine du dépôt**, jamais depuis `tools/`) :
  ...
  - `tools/propose_ktiv_male.js` — propose la graphie **pleine** (ktiv male) d'un mot vocalisé, et compte combien de formes du carnet s'en écartent. Lecture seule : il PROPOSE, il ne décide pas — aucune graphie pleine n'étant écrite à la main dans `data/`, il n'existe rien contre quoi valider une règle. Sa sortie sépare ce qui est mécanique de ce qui demande un arbitrage
...
- `docs/` — la prose du projet (non déployée) : `ARCHITECTURE.md` (flux de données et contrats), `DESIGN.md` (la charte visuelle), `PRODUCT.md` (registre et intentions), `RITUEL.md` (le rituel après une modification, et l'outillage), `TODO.md` (état courant seulement), `SPEC_AJOUTE_MOTS.md` (le contrat du générateur de fiches), `TODO_ARCHIVE.md` (l'archive — tout le récit du projet y vit, et lui seul), `lots-en-attente/` (les bordereaux de vocabulaire bloqués, en attente d'arbitrage) et `superpowers/` (les specs et plans du chantier en cours)
