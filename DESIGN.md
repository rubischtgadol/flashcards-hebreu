...
⚠️ **`--colonne-large` est un plancher mesuré, pas un confort.** La table la plus large se pose naturellement à 890px et aucune ne dépasse 894px. Le resserrer à 44rem « puisque les tables portent `min-width:640px` » met **7 tables sur 29 en défilement horizontal** : ce `min-width` est lui aussi un plancher, qui ne joue que sur mobile. Essayé, mesuré, annulé. Recontrôlé après le lot grammaire du 21/07 : 36 tables, la plus large des cinq nouvelles à 806px — le plancher tient (4 sur 36 débordent à 880px). **Recalibré une seconde fois le 21/07, après les lots argent-achats et loisirs-culture : 42 tables, et pour la première fois une qui dépassait** — « Vie quotidienne » (Verbes) à **909,94px**, corrigée depuis ; les 41 autres exactement à 894,00. Le plancher n'a pas bougé, mais la phrase « aucune ne dépasse 894px » est restée fausse pendant un lot : elle décrivait une mesure, pas une garde.
...
- **relever `--colonne-large`** : déplacerait sommaire, titres à filet et 42 tables pour arranger un cas (piège n°4) ;
...
### Do:
- **Do** modifier les tokens **dans `src/tokens.css`** (source unique, injectée dans le carnet et dans l'app au build) et reporter à la main dans `index.html` seul — le bloc `:root` doit rester strictement identique dans les trois ; si `--bg`/`--gold` changent, régénérer les icônes et `manifest.webmanifest`.
