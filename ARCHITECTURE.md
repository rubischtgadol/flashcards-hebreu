...
(`firstSpanText` dans [build.js](build.js)) cherchant `class="he"` par correspondance exacte et tolérant les
...
7. **Hook `pre-commit` versionné** ([.githooks/pre-commit](.githooks/pre-commit), 25/07 ; installation par machine : `git config core.hooksPath .githooks`) : exécute `node build.js --check` et `node verifie_exemples.js` avant chaque commit, et refuse un commit qui change un fichier servi (`app.html`, les artefacts, `index.html`, `manifest.webmanifest`, `sw.js`, `icons/`) sans bump de `VERSION` dans `sw.js` (piège n°10 ; bypass assumé : `--no-verify`). Tripwire de transition — le Task 19 fera estampiller `VERSION` par le build.
