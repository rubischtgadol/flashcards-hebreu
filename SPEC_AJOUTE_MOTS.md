...
**Les fautes reproductibles d'`he2tr`, relevées sur les deux lots d'adjectifs du
24/07 (26 `tr` fournis à la main sur 200 dérivés)** — à chercher en priorité dans
le tableau, le `⚠` ne les couvre pas toutes :

| Ce qu'`he2tr` écrit | Ce qu'il faut | Cas |
| --- | --- | --- |
| `shekufah`, `shetuchah`, `shekhichah`, `penuyah` | `shkufah`, `shtuchah`, `shkhichah`, `pnuyah` | shva initial devant sifflante — le carnet écrit le groupe (`shketah`, `shkhenah`, `gdolim`) |
| `meiuman`, `veiafah`, `beiisra'el` | `meyuman`, `veyafah`, `beyisra'el` | yud consonantique rendu `i` |
| `boddim`, `chiurim` | `bodedim`, `chivrim` | redoublement mal résolu (בּוֹדְדִים, חִוְּרִים) |
| `achray`, `kefuot` | `achra'i`, `kefu'ot` | alef final : apostrophe perdue |

Le shva initial reste un **jugement** (`gdolim` mais `ledaber`) : `nedirah`,
`zehirah`, `gemishah` sont laissés tels quels. La règle pratique : surcharger
quand le carnet écrit déjà le groupe pour la même consonne.
